...
from app.services.settlement_service import get_fee_rates, get_sales_commission_rate
...
def designer_settlement(
    ...
    본인 귀속 매출 → PG수수료 → 분배가능액 → 디자이너 몫(본인 분배율).
    영업수수료는 PG수수료 내에서 차감되므로 분배가능액 산정에는 영향 없음.
    ...
    pg_fee_rate, _ = get_fee_rates(db, staff.merchant_id)
